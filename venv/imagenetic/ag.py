...
BITS_MUTACION = int(CHROMOSOME_LENGTH * PORCENTAJE_MUTACION)
...
def fitness_func(individuo, target_img=TARGET_IMAGE):
    mse = np.mean(np.abs(target_img - individuo))
    return mse
...
def selection_operator(survivor_percentage, poblacion):
    ...
    sorted_individuals = sorted(indivuals_w_scores, key=lambda x: x[1])
    ...
    for individuo in poblacion:
        ...
        if loteria == 50:
            ...
            for _ in range(bits):
                ...
                individuo[random_bit] = np.random.rand()
...
axes[1].imshow(best_solution)
axes[1].axis('off')  # Turn off axis for the second subplot
axes[1].set_title('Best evolution')
